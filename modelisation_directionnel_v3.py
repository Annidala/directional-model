...
n=np.array([0.,np.cos(np.pi/3.),np.cos(np.pi/3),0.,-np.cos(np.pi/3),-np.cos(np.pi/3)]) #exemple avec seulement 6 directions
t=np.array([1.,np.sin(np.pi/3.),-np.sin(np.pi/3),-1.,-np.sin(np.pi/3),np.sin(np.pi/3)])
z=np.array([0.,0.,0.,0.,0.,0.])

### orientations star
nstar=np.array([1.,np.cos(np.pi/6.),np.cos(np.pi/6.),-1.,-np.cos(np.pi/6.),-np.cos(np.pi/6.)]) #exemple avec seulement 6 directions
tstar=np.array([0.,np.sin(np.pi/6.),-np.sin(np.pi/6.),0.,-np.sin(np.pi/6.),np.sin(np.pi/6.)])
...
def err_matrice(p,stress_1, strain, u1m=u1m, u2m=u2m, u3m=u3m):
	...
	for j in range(len(strain)):
		...
		Wm1=[1/12.*(strain[j]*u1m[i]**2.-(strain[j])**(-2.)*u2m[i]**2.)*(grospate(p[0],p[1],vi_bis(mat_strain,0,u1m[i],u2m[i],u3m[i])/np.sqrt(p[1]))) for i in range(len(u1m))]
		Wm2=[1/12.*(strain[j]*u2m[i]**2.-(strain[j])**(-2.)*u1m[i]**2.)*(grospate(p[0],p[1],vi_bis(mat_strain,np.pi/2,u1m[i],u2m[i],u3m[i])/np.sqrt(p[1]))) for i in range(len(u1m))]
...
strain=np.arange(1.0,1.5,0.01)
### matrice
stress_matrice_dragon=model_MR2(strain_matrice,0.014,0.0007)
stress_matrice_SC37=model_MR2(strain_matrice,0.086,0.003)
### composite
stress_sens1 = model_MR2(strain, CO_dragon[0],C1_dragon[0])
stress_sens2 = model_MR2(strain, CO_dragon[1],C1_dragon[1])
...
for j in range(len(strain_matrice)):
	...
	Wm1=[1/12.*(strain_matrice[j]*u1m[i]**2.-(strain_matrice[j])**(-2.)*u2m[i]**2.)*(
		grospate(coeff_dragon[0],coeff_dragon[1],vi_bis(mat_strain,0,u1m[i],u2m[i],u3m[i])/np.sqrt(coeff_dragon[1]))) for i in range(len(u1m))]
	Wm2=[1/12.*(strain_matrice[j]*u1m[i]**2.-(strain_matrice[j])**(-2.)*u2m[i]**2.)*(
		grospate(coeff_SC37[0],coeff_SC37[1],vi_bis(mat_strain,0,u1m[i],u2m[i],u3m[i])/np.sqrt(coeff_SC37[1]))) for i in range(len(u1m))]
	dragon[j]=sum(Wm1)
	SC37[j]=sum(Wm2)
...
def err_wifix(p,stress_1, stress_2, strain, C, N, u1m=u1m, u2m=u2m, u3m=u3m, n=n, t=t, u3=z, nstar=nstar, tstar=tstar, zstar=zstar):
	...
	wm=np.array([p[4],p[4],p[4],p[4],p[4],p[4]])
	
	for j in range(len(strain)):
		mat_strain=np.array([[strain[j],0,0],[0,1/np.sqrt(strain[j]),0],[0,0,1/np.sqrt(strain[j])]])
		###matrice : elastomere isotrope
		Wm1=[1/12.*(strain[j]*u1m[i]**2.-(strain[j])**(-2.)*u2m[i]**2.)*(grospate(C,N,vi_bis(mat_strain,0,u1m[i],u2m[i],u3m[i])/np.sqrt(N))) for i in range(len(u1m))]
		### tricot imprégné sens 1 
		Wt1=[wi[i]*np.sqrt(N_mat[i])*(1/vi_struct(mat_strain,0,n,t,z)[i])*(strain[j]*n[i]**2.-(strain[j])**(-2.)*t[i]**2.)*(
			approx_boyce(N_mat[i],vi_struct(mat_strain,0,n,t,z)[i])) for i in range(len(n))]
		Wl1=[1/12.*(strain[j]*nstar[i]**2.-(strain[j])**(-2.)*tstar[i]**2.)*(
			grospate(C,N,vi_bis(mat_strain,0,nstar[i],tstar[i],zstar[i])/np.sqrt(N))) for i in range(len(nstar))]
		### tricot imprégné sens 2
		Wt2=[wi[i]*np.sqrt(N_mat[i])*(1/vi_struct(mat_strain,np.pi/2,n,t,z)[i])*(strain[j]*t[i]**2.-(strain[j])**(-2.)*n[i]**2.)*(
			approx_boyce(N_mat[i],vi_struct(mat_strain,np.pi/2,n,t,z)[i])) for i in range(len(n))]
		Wl2=[1/12.*(strain[j]*tstar[i]**2.-(strain[j])**(-2.)*nstar[i]**2.)*(
			grospate(C,N,vi_bis(mat_strain,np.pi/2,nstar[i],tstar[i],zstar[i])/np.sqrt(N))) for i in range(len(nstar))]
		func_sens1[j]=phi*(sum(Wt1)+sum(Wl1))+(1-phi)*sum(Wm1)
		func_sens2[j]=phi*(sum(Wt2)+sum(Wl2))+(1-phi)*sum(Wm1)
	
	res_sens1=stress_1-func_sens1+func_sens1[0]
	res_sens2=stress_2-func_sens2+func_sens2[0]
	return sum(res_sens1*res_sens1+res_sens2*res_sens2/corr)
...
bnds = ((0,None), (0.01,None), (0,None),(0,None),(0,None),(0,1))
p0=[0.1,0.1,5,50,0.1,0.2] # Cc, Nc, Ce, Ne, Fc, Fe

P=opt.minimize(err_wifix,p0,args=(stress_sens1, stress_sens2,strain,coeff_dragon[0],coeff_dragon[1],u1m,u2m,u3m,n,t,z,nstar,tstar,zstar),method='SLSQP',bounds=bnds,constraints=cons)
coeff=P.x
...
for j in range(len(strain)):
	...
	Wl1=[1./12*(strain[j]*nstar[i]**2.-(strain[j])**(-2.)*tstar[i]**2.)*(
		grospate(C,N,vi_bis(mat_strain,0,nstar[i],tstar[i],zstar[i])/np.sqrt(N))) for i in range(len(nstar))]
	### sens 2
	Wt2=[wi[i]*np.sqrt(N_mat[i])*(1/vi_struct(mat_strain,np.pi/2,n,t,z)[i])*(
		strain[j]*t[i]**2.-(strain[j])**(-2.)*n[i]**2.)*(approx_boyce(N_mat[i],vi_struct(mat_strain,np.pi/2,n,t,z)[i])) for i in range(len(n))]
	Wl2=[1./12*(strain[j]*tstar[i]**2.-(strain[j])**(-2.)*nstar[i]**2.)*(
		grospate(C,N,vi_bis(mat_strain,np.pi/2,nstar[i],tstar[i],zstar[i])/np.sqrt(N))) for i in range(len(nstar))]
	estim[j]=phi*(sum(Wt1)+sum(Wl1))+(1-phi)*sum(Wm1)
	estim_trame[j]=phi*(sum(Wt2)+sum(Wl2))+(1-phi)*sum(Wm1)
	tricot_t[j]=sum(Wt1)
	tricot_c[j]=sum(Wl1)
...
plt.grid();plt.legend(loc=5,prop={'family': 'serif', 'size':15});plt.ylabel('Stress (MPa)');plt.xlabel('Strain (mm/mm)');
...
for j in range(len(strain)):
	...
	mat_strain=np.array([[strain[j],0,0],[0,1/np.sqrt(strain[j]),0],[0,0,1/np.sqrt(strain[j])]])
	###matrice : elastomere isotrope
	Wm1=[1/12.*(strain[j]*u1m[i]**2.-(strain[j])**(-2.)*u2m[i]**2.)*(grospate(C2,N2,vi_bis(mat_strain,0,u1m[i],u2m[i],u3m[i])/np.sqrt(N2))) for i in range(len(u1m))]
	### sens 1 
	Wt1=[wi[i]*np.sqrt(N_mat[i])*(1/vi_struct(mat_strain,0,n,t,z)[i])*(strain[j]*n[i]**2.-(strain[j])**(-2.)*t[i]**2.)*(
		approx_boyce(N_mat[i],vi_struct(mat_strain,0,n,t,z)[i])) for i in range(len(n))]
	Wl1=[1/12.*(strain[j]*nstar[i]**2.-(strain[j])**(-2.)*tstar[i]**2.)*(
		grospate(C2,N2,vi_bis(mat_strain,0,nstar[i],tstar[i],zstar[i])/np.sqrt(N2))) for i in range(len(nstar))]
	### sens 2
	Wt2=[wi[i]*np.sqrt(N_mat[i])*(1/vi_struct(mat_strain,np.pi/2,n,t,z)[i])*(
		strain[j]*t[i]**2.-(strain[j])**(-2.)*n[i]**2.)*(approx_boyce(N_mat[i],vi_struct(mat_strain,np.pi/2,n,t,z)[i])) for i in range(len(n))]
	Wl2=[1/12.*(strain[j]*tstar[i]**2.-(strain[j])**(-2.)*nstar[i]**2.)*(
		grospate(C2,N2,vi_bis(mat_strain,np.pi/2,nstar[i],tstar[i],zstar[i])/np.sqrt(N2))) for i in range(len(nstar))]
...
#plt.figure()
plt.plot(strain, model_MR2(strain, C0_SC37[1],C1_SC37[1]), 'b', label = 'Exp: Warp direction')
...
plt.figure()
plt.plot(strain, abs((model_MR2(strain, C0_SC37[0],C1_SC37[0])-(model_compBc-model_compBc[0]))/model_MR2(strain, C0_SC37[0],C1_SC37[0])), 'g')
#plt.show()
