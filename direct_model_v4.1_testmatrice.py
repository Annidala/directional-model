...
def err_matrice(p,stress=stress_, strain=strain_, u=v, wi=wi):
	res_sens1=np.zeros_like(stress_)
	res_sens2=np.zeros_like(stress_)
	for j in range(len(strain)):
		strain=_toTensor(strain_[j], 0, 1/np.sqrt(strain_[j]), 1/np.sqrt(strain_[j]) ) ## def of strain tens at j
		...
		strain0=_toBase(strain, 0,True)
		invstrain0=_toBase(invstrain, 0,True)
		stress0=_toBase(stress, 0)
		sig0_11=sum([(wi[i])*p[0]*_approxLangevin(_computeDirectionStrain(strain0,u[:,i])/np.sqrt(p[1]),'Boyce')*_computeDeriveDirectionStrain(strain0, u[:,i]) for i in range(u.shape[1])])
		...
		strain90=_toBase(strain, np.pi/2., True)
		invstrain90=_toBase(invstrain, np.pi/2.,True)
		stress90=_toBase(stress, np.pi/2.)
		sig90_11=sum([(wi[i])*p[0]*_approxLangevin(_computeDirectionStrain(strain90,u[:,i])/np.sqrt(p[1]),'Boyce')*_computeDeriveDirectionStrain(strain90, u[:,i]) for i in range(u.shape[1])])
...
P_dragon=opt.minimize(err_matrice,p0,args=(stress_, strain_,u,wi),method='SLSQP')
...
plt.plot(strain_, stress_,'b', linewidth=5);plt.plot(strain_, contrainte_mod,'r--', linewidth=5);plt.plot(strain_, contrainte_mod2, 'g')
